...
new #[Title('Product Detail | E-Commerce')] class extends Component {
    public $slug;
    ...
    <section class="overflow-hidden bg-white py-11 font-poppins dark:bg-gray-800">
        <div class="max-w-6xl px-4 py-4 mx-auto lg:py-8 md:px-6">
            <div class="flex flex-wrap -mx-4">
                <div class="w-full mb-8 md:w-1/2 md:mb-0" x-data="{ mainImage: '{{ url('storage', $this->product->images[0]) }}' }">
                        ...
                        <div class="flex-wrap hidden md:flex ">
                            @foreach ($this->product->images as $image)
                                <div class="w-1/2 p-2 sm:w-1/4" x-on:click="mainImage='{{ url('storage', $image) }}'">
                                    <img src="{{ url('storage', $image) }}" alt="{{ $this->product->name }} image"
                ...
                <div class="w-full px-4 md:w-1/2 ">
                    <div class="lg:pl-20">
                        <div class="mb-8 [&_ul]:list-disc [&_ul]:ml-4">
                            <h2 class="max-w-xl mb-6 text-2xl font-bold dark:text-gray-400 md:text-4xl">
                                {{ $this->product->name }}</h2>
                            <p class="inline-block mb-6 text-4xl font-bold text-gray-700 dark:text-gray-400 ">
                                <span>{{ Number::currency($this->product->price, 'AFN') }}</span>
                                {{-- <span class="text-base font-normal text-gray-500 line-through dark:text-gray-400">$1800.99</span> --}}
                            </p>
                            <div class="max-w-md whitespace-normal break-words text-gray-700 dark:text-gray-400">
                                {!! Str::markdown($this->product->description) !!}
                            </div>
                        </div>
                        <div class="w-32 mb-8 ">
                            <label for=""
                        ...
                        <div class="flex flex-wrap items-center gap-4">
                            <button wire:click='addToCart({{ $this->product->id }})'
                                ...
                                <span wire:loading.remove wire:target='addToCart({{ $this->product->id }})'>Add to cart</span>
                                <span wire:loading wire:target='addToCart({{ $this->product->id }})'>Adding...</span>
                            </button>
                        </div>
                    </div>
                </div>
